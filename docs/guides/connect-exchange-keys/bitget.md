# Bitget

Use this page to create a Bitget API key for SignalHorse.

Open the Bitget API creation page here:

`https://www.bitget.com/account/newapi`

## Create the key

1. Sign in to Bitget and start the API creation flow.

![Bitget API creation form](../../assets/exchange-api/bitget-create-form.png)

2. Enter an API remark name and the API password required by Bitget.

![Bitget API password](../../assets/exchange-api/bitget-api-password.png)

3. Enable the necessary spot and swap trading permissions only. Do not enable withdrawal or wallet-transfer-related permissions.

![Bitget permissions](../../assets/exchange-api/bitget-permissions.png)

4. Finish the creation and copy the API key and secret key displayed by Bitget.

![Bitget created API keys](../../assets/exchange-api/bitget-created-keys.png)

Bitget also requires the passphrase or API password later in SignalHorse, so keep that value with the key pair.

Then continue to [Add the Keys to SignalHorse](signalhorse.md).